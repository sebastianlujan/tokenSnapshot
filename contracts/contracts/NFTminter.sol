...
contract Foo is ERC1155(''), Ownable {
	...
	function setEthereanEmpireContractAddress(
	...
	function mint(address _minter, uint _dropId) external {
		require(
			msg.sender == ETHEREAN_EMPIRE_CONTRACT_ADDRESS,
			'Can only mint through Etherean Empire mintDrop function.'
		);
		require(MINT_ACTIVE, 'Minting is not active.');
